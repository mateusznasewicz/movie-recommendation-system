...
public class QualityMeasure {
    public static <T extends Entity, G extends Entity> double MAE(double[][] predicted, RatingService<T,G> ratingService, boolean training){
        double error = 0;
        int n = 0;
        int total = 0;
        for(int u: ratingService.getEntitiesID()){
            ...
            if(training){
                ratings = ratingService.getEntity(u).getRatings();
            }else{
                ratings = ratingService.getEntity(u).getTestRatings();
